...
/// Whether the active palette reads as dark. Derived from the background
/// lightness rather than a stored flag, so it follows preset *and* custom
/// themes without extra plumbing.
pub fn is_dark() -> bool {
    palette().background.l < 0.5
}

/// Resolve a light/dark pair against the active palette. Exposed for modules
/// that keep their own local colors and only need to know which way to lean.
...
/// search fields, selected chips and floating controls. Use
/// [`chat_surface_color`] instead for surfaces that sit over a background image.
pub fn surface() -> Hsla {
    palette().surface
}
